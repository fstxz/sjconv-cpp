...
int process (jack_nframes_t nframes, void *arg)
{
	for (int i = 0; i < input_ports.size(); i++) {
    	auto in = (jack_default_audio_sample_t *)jack_port_get_buffer (input_ports[i], nframes);
    	auto out = (jack_default_audio_sample_t *)jack_port_get_buffer (output_ports[i], nframes);
...
int main(int argc, char *argv[]) {
    ...
    if (!sndfile) {
        std::cerr << "Error opening \"" << filepath << "\": " << sf_strerror(sndfile) << std::endl;
        return 1;
    }
    ...
    if (frames_read != sfinfo.frames) {
        std::cerr << "Error reading all frames. Read: " << frames_read << " Expected: " << sfinfo.frames << std::endl;
    }
    ...
    if (sfinfo.samplerate != jack_sample_rate) {
        std::cerr << "Sample rate of the inpulse response must match the sample rate of the JACK server" << std::endl;
        return 1;
    }
	...
	for (int i = 0; i < port_num; i++) {
	    auto in_port = jack_port_register (client, ("input." + std::to_string(i+1)).data(), JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput, 0);
